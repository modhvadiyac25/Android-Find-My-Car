...
class UserProfile : AppCompatActivity() {
    ...
    lateinit var imagePathFromStorage : String
    private lateinit var imageUri : Uri
    private val REQUEST_IMAGE_CAPTURE = 100
    var database: DatabaseReference =FirebaseDatabase.getInstance().getReference("users")
    var uid = FirebaseAuth.getInstance().currentUser?.uid
    val storageReference = FirebaseStorage.getInstance().reference.child("pics").child(uid!! + ".jpeg")

  //  var httpRefrence = FirebaseStorage.getInstance().getReferenceFromUrl(imagePathFromStorage)
   ...
   // private var activity = getActivity()
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       Glide.with(this)?.load(storageReference)?.into(image_view)
